...
1. 상환 이력 없는 고객군 내부에서 거래 이력 구간별 미상환율 확인
2. 상품군별 전체 대출 건수와 미상환 건수 비교
3. 리테일 상품군과 대출 공급자 2의 연결 확인
4. 거래 이력 없음 그룹에 리테일·대출 공급자 2가 몰려 있는지 확인
5. 리테일 또는 대출 공급자 2를 제외했을 때 거래 이력 없음 그룹의 미상환율 재계산
6. 서비스 흐름 ID별 상품군 구성과 미상환 집중도 확인
...
EDA에서는 그림을 적극적으로 사용합니다. 이 프로젝트에서 README에 포함하면 좋은 핵심 그림은 다음과 같습니다.

| 그림 | 보여줄 내용 | README에서의 역할 |
|---|---|---|
| 상환 이력 유무별 대출/미상환 분포 | 전체 대출은 상환 이력 있는 고객군이 많지만, 미상환은 상환 이력 없는 고객군에 집중 | 왜 상환 이력 없는 고객군을 따로 봐야 하는지 설명 |
| 거래 이력 구간별 미상환율 | 거래 이력 없음 그룹의 미상환율이 높음 | 거래 행동이 처음에는 유망한 신호처럼 보였다는 점 설명 |
| 상품군별 대출 건수 vs 미상환 건수 | 리테일에 미상환이 비정상적으로 집중 | 고객 행동 신호가 상품군 구조와 얽혀 있음을 보여줌 |
...
이 프로젝트의 Evaluation은 네 가지 흐름으로 정리할 수 있습니다.

1. EDA에서는 거래 행동이 위험 신호처럼 보였는가?
2. 그 신호는 상품군·대출 공급자·서비스 흐름 구조를 제거해도 남는가?
3. 모델에 거래 행동 변수를 추가했을 때 실제 성능이 개선되는가?
4. 운영상 중요한 고위험 구간 포착력이 개선되는가?

---

### 6.1 EDA: 거래 이력 없음은 처음에는 강한 위험 신호처럼 보였다
...
이 결과만 보면 거래 행동은 상환 이력이 없는 고객의 위험을 설명하는 유망한 신호처럼 보입니다.
...
### 6.2 EDA: 높은 미상환율은 리테일·대출 공급자 2·서비스 흐름 ID 6과 얽혀 있었다

거래 이력 없음 그룹의 높은 미상환율을 더 확인하기 위해 상품군별 대출 건수와 미상환 건수를 비교했습니다.

전체 대출 건수만 보면 통신요금 상품군이 가장 많았지만, 미상환은 리테일 상품군에 크게 집중되어 있었습니다.

| 관찰 결과 | 해석 |
|---|---|
| 전체 대출 건수는 통신요금 상품군이 가장 많음 | 대출 규모만 보면 통신요금이 주요 상품군처럼 보임 |
| 미상환은 리테일 상품군에 집중 | 단순 규모가 아니라 손실 집중 상품군을 봐야 함 |
| 리테일은 대출 공급자 2와 강하게 연결 | 상품군 효과와 공급자 구조가 함께 작동할 가능성 |
| 거래 이력 없음 그룹에 리테일·대출 공급자 2가 상대적으로 많이 포함 | 거래 행동 신호가 구조적 세그먼트의 대리 신호일 수 있음 |

리테일과 대출 공급자 2를 제외하면 거래 이력 없음 그룹의 미상환율은 크게 낮아졌습니다.
...
서비스 흐름 ID 분석에서도 같은 구조가 확인되었습니다.
...
### 6.3 모델 검증: 거래 행동 변수 추가는 성능을 안정적으로 개선하지 못했다
...
### 6.4 Permutation 검증: 실제 성능을 지탱한 것은 거래 행동보다 금액·시간·서비스 흐름이었다
...
### 6.5 Score Band: 거래 행동 변수를 추가해도 고위험 구간 포착력은 개선되지 않았다
